...
#DEFINE C_CANT_GENERATE_FILE_BECAUSE_IT_IS_READONLY_LOC		'Can't generate file [<<THIS.c_OutputFile>>] because it is ReadOnly'
#DEFINE C_CONFIGFILE_LOC									'ConfigFile:'
...
#DEFINE C_END_MARKER_NOT_FOUND_LOC							'Can't found end marker [<<ta_ID_Bloques(lnPrimerID,2)>>] that closes start marker [<<ta_ID_Bloques(lnPrimerID,1)>>] on line <<TRANSFORM(taBloquesExclusion(tnBloquesExclusion,1))>>'
...
#DEFINE C_STRUCTURE_NESTING_ERROR_ENDPROC_EXPECTED_LOC		'Nesting structure error. ENDPROC expected but found ENDDEFINE on class <<toClase._Nombre>> (<<toObjeto._Nombre>>.<<loProcedure._Nombre>>), line <<TRANSFORM(I)>> of file <<THIS.c_InputFile>>'
